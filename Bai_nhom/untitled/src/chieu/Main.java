...
public class Main {
    ...
    private static void initializeSampleData() {
        ...
        Lecturer lecturer1 = new Lecturer("L001", "Dr. John Smith", 45, "john.smith@university.edu", 40, "Computer Science");
        Lecturer lecturer2 = new Lecturer("L002", "Prof. Sarah Johnson", 38, "sarah.johnson@university.edu", 35, "Software Engineering");
        Lecturer lecturer3 = new Lecturer("L003", "Dr. Michael Brown", 42, "michael.brownnn23@university.edu", 38, "Database Systems");
        Lecturer lecturer4 = new Lecturer("L004", "Dr. Michael Brown", 42, "brown.michael1989@university.edu", 38, "Object Oriented Programming");
        Lecturer lecturer5 = new Lecturer("L005", "Prof. Sarah Johnson", 42, "johnson.avetiger@university.edu", 38, "CODIO");

        // Add Backend Students
        persons.add(new StudentBE("BE001", "Alice Chen", 22, "alice.chen@student.edu", 8.7, 45, "Java"));
        persons.add(new StudentBE("BE002", "Bob Wilson", 23, "bob.wilson@student.edu", 7.2, 42, "Python"));
        persons.add(new StudentBE("BE003", "Bob Wilson", 21, "charlie.davis@student.edu", 6.8, 38, "C#"));
        persons.add(new StudentBE("BE004", "Diana Lee", 24, "diana.lee@student.edu", 9.1, 48, "Java"));

        // Add Fullstack Students
        persons.add(new StudentFS("FS001", "Eva Martinez", 23, "eva.martinez@student.edu", 8.3, 50, 3));
        persons.add(new StudentFS("FS002", "Frank Thompson", 22, "frank.thompson@student.edu", 7.9, 47, 2));
        persons.add(new StudentFS("FS003", "Grace Kim", 25, "grace.kim@student.edu", 8.8, 52, 5));
        persons.add(new StudentFS("FS004", "Grace Kim", 24, "henry.garcia@student.edu", 6.5, 44, 1));
        ...
        TeachingAssistant ta1 = new TeachingAssistant("TA001", "Ivan Petrov", 26, "ivan.petrov@university.edu", 20, 15);
        ta1.addLecture(lecturer1);
        ta1.addLecture(lecturer2);

        TeachingAssistant ta2 = new TeachingAssistant("TA002", "Julia Wang", 25, "julia.wang@university.edu", 18, 12);
    ...
    private static void mainMenu() {
        ...
        System.out.println("16. Kiểm tra lớp học có học viên bị trùng");
        System.out.println("17. Kiểm tra lịch dạy của giảng viên có bị trùng");
        System.out.println("18. Tìm học viên theo ID");
        System.out.println("19. Tra cứu lớp học theo maLop");
        System.out.println("20. Quản lý lịch dạy theo từng giảng viên");
    ...
    // 17. Kiểm tra buổi dạy của giảng viên có trùng nhau không
    private static void checkScheduleHasDuplicateLecture() {
        Set<String> uniqueNames = new HashSet<>();
        Set<String> duplicates = new HashSet<>();
    ...
    // 18. Tìm học viên theo ID
    private static void findStudentById() {
        Map<String, Student> studentMap = new HashMap<>();

        for (Person p : persons) {
            if (p instanceof Student) {
                studentMap.put(p.getId(), (Student) p);
            }
        }

        System.out.print("Nhập ID của học viên cần tìm: ");
        String id = sc.nextLine().toUpperCase().trim();

        if (studentMap.containsKey(id)) {
            System.out.println("+) Học viên có " + studentMap.get(id));
        } else {
            System.out.println("+) Không tìm thấy học viên với ID: " + id);
        }
    }

    // 19. Tìm lớp học theo maLop
    private static void findCourseById() {
        System.out.print("Nhập mã lớp cần tra cứu: ");
        String idClass = sc.nextLine();

        boolean found = false;
        for (Course c : courses) {
            if (c.getId().equalsIgnoreCase(idClass)) {
        ...
        if (!found) {
            System.out.println("Không tìm thấy lớp học với mã: " + idClass);
        }
    }
